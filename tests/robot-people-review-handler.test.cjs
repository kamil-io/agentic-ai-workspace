...
const runParser=raw=>new Function('$input',node('Validate Telegram Review').parameters.jsCode.replace('YOUR_TELEGRAM_REVIEWER_ID','765967664').replace('YOUR_TELEGRAM_CHAT_ID','765967664'))({first:()=>({json:raw})});
...
test('callback parser accepts only the trusted reviewer, chat and action format',()=>{
 ...
 assert.deepEqual(runParser(valid)[0].json,{action:'approve',draftId:'12',reviewerId:'765967664',chatId:'765967664',messageId:44,callbackQueryId:'cb-1'});
 for(const bad of [
